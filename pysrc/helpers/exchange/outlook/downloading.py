...
def _api_request_binary(
    ...
    if not resp.ok:
        print(colored(f"Binary request failed: {route} ({resp.status_code})", "red"))
        return None
...
def _export_message_from_data(
    ...
    for att in attachments:
        ...
        if attachment_type == "referenceAttachment":
            if attachment_id:
                _write_json(os.path.join(attachments_links_dir, f"{attachment_shortcode}.json"), att)
            files_map.append({
        ...
        if attachment_type == "itemAttachment":
            ...
            if item:
                ...
                if "message" in item_type:
                    _export_message_from_data(item, item_dir, item.get("id"), allow_graph_attachments=True)
    ...
    _write_json(
        ...
        {"shortcodeLength": attachment_shortcode_length, "shortcodeToId": attachment_shortcode_to_id},
    )
...
def _load_folder_forest() -> Optional[List[Dict[str, Any]]]:
    ...
    if not os.path.isfile(folders_path):
        print(colored(f"Missing {folders_path}. Run indexing first.", "red"))
        return None
...
def _load_conversations(
    ...
    new_path = os.path.join(INDEX_ROOT, "conversations-organized", f"{folder_shortcode}.json")
    old_path = os.path.join(INDEX_ROOT, "conversations-organized", f"{folder_id}.json")
    if not os.path.isfile(new_path) and os.path.isfile(old_path):
        with open(old_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, list):
            data = {"folderId": folder_id, "folderShortcode": folder_shortcode, "conversations": data}
        _write_json(new_path, data)
    if not os.path.isfile(new_path):
        print(colored(f"Missing conversation index for folder {folder_id}. Run indexing first.", "red"))
        return None
...
def _ensure_conversation_shortcodes(
    ...
    conv_id_to_shortcode, conv_shortcode_to_id, conv_length = build_shortcode_map(conversation_ids)
    ...
    for conversation in conversations:
        ...
        msg_id_to_shortcode, msg_shortcode_to_id, msg_length = build_shortcode_map(message_ids)
...
def download_all_folders(reset: bool = False) -> int:
    ...
    _write_json(os.path.join(INDEX_ROOT, "folders.json"), forest)
    ...
    for i, (folder_name, node) in enumerate(folders):
        ...
        conversations = _ensure_conversation_shortcodes(conversations_data, conversations_path)
        ...
        with tqdm(total=total_items, desc=desc) as pbar:
            for conversation in conversations:
                ...
                for message_meta in conversation.get("messages", []):
                    ...
                    if not ok:
                        print(colored(
                            f"Failed to export message {message_id} in folder {folder_name}", "red"
                        ))
                        return -1
                    pbar.update(1)
