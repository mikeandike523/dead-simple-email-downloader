...
def index_folder_get_top_level_ids(node):
    ...
    target_path = _resolve_index_file(
        os.path.join(INDEX_ROOT, "top-level-messages"), folder_id, folder_shortcode
    )
    if not os.path.isfile(target_path):
        ...
        with open(target_path, "w", encoding="utf-8") as f:
            ...
            print(colored(f"Indexed messages in folder {node['name']} ({folder_id}).", "green"))
    return True
...
def index_folder_sanity_check(node):
    ...
    if not os.path.isfile(top_level_messages_path):
        print(colored(
            f"No top-level messages found in folder {node['name']} ({folder_id}). Fatal error.", "red"
        ))
    ...
    if abs(indexedItemCount - totalItemCount) > INDEX_SANITY_CHECK_MAX_DISCREPANCY:
        print(colored(
            f"Error: Index discrepancy of at least {INDEX_SANITY_CHECK_MAX_DISCREPANCY} detected!", "red"
        ))
        return False
    print("Checking id uniqueness...")
    message_ids_set = set(message_ids)
    if len(message_ids) != len(message_ids_set):
        print(colored("Error: Duplicated message IDs detected!", "red"))
        return False
...
def index_folder_get_top_level_metadata(folder_name, node):
    ...
    metadata_path = _resolve_index_file(
        os.path.join(INDEX_ROOT, "top-level-message-metadata"), folder_id, folder_shortcode
    )
...
Try resetting the index and running indexing again.
        ...
        with open(messages_path, "r", encoding="utf-8") as f:
            message_ids = json.load(f)
        message_ids_chunked = [
            message_ids[i:i + INDEX_GET_METADATA_CHUNK_SIZE]
...
""", "red"))
    ...
    out_dir = os.path.join(INDEX_ROOT, "conversations-organized")
    ...
    print(colored(
        f'Organized {len(result)} conversation group(s) for folder "{folder_name}" -> {output_path}',
        "green",
    ))
    return True
