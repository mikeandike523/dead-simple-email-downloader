...
from pysrc.helpers.exchange.paths import EXCHANGE_OUTLOOK_INDEX as INDEX_ROOT, EXCHANGE_OUTLOOK_CACHE as CACHE_ROOT
...
def _message_datetime(meta: Dict[str, Any]) -> datetime:
    dt = _parse_iso_datetime(meta.get("receivedDateTime")) or _parse_iso_datetime(meta.get("sentDateTime"))
    if dt:
        return dt
...
def _load_folder_forest() -> Optional[List[Dict[str, Any]]]:
    ...
    if not os.path.isfile(folders_path):
        print(colored(f"Missing {folders_path}. Run 'dsed exchange outlook index' first.", "red"))
        return None
...
def _load_folder_conversations(folder_shortcode: str) -> Optional[Dict[str, Any]]:
    path = os.path.join(INDEX_ROOT, "conversations-organized", f"{folder_shortcode}.json")
    if not os.path.isfile(path):
        print(colored(
            f"Missing conversation index for folder shortcode {folder_shortcode}. "
            "Run 'dsed exchange outlook index' first.",
            "red",
        ))
        return None
...
def export_outlook_output(outdir: str, max_subject_chars: int = 36) -> int:
    ...
    if not os.path.isdir(CACHE_ROOT):
        print(colored(f"Missing {CACHE_ROOT}. Run 'dsed exchange outlook download' first.", "red"))
        return -1

    _ensure_dir(outdir)

    folder_shortcodes = None
    shortcodes_path = os.path.join(INDEX_ROOT, "shortcodes", "folders.json")
    if os.path.isfile(shortcodes_path):
        with open(shortcodes_path, "r", encoding="utf-8") as f:
    ...
    for parents, node in folders:
        ...
        folder_index_payload = {
            ...
            "conversationShortcodeLength": conversations_data.get("conversationShortcodeLength"),
            "outputFolder": os.path.relpath(folder_out_dir, outdir),
            "cacheFolder": os.path.relpath(os.path.join(CACHE_ROOT, folder_shortcode), "."),
            "conversations": [
                {
        ...
        folder_cache_dir = os.path.join(CACHE_ROOT, folder_shortcode)
        ...
        for conv_index, conversation in enumerate(conversations, start=1):
            ...
            conv_subject_raw = first_meta.get("subject") if isinstance(first_meta, dict) else None
            conv_subject = _truncate_subject(
                _safe_filename(conv_subject_raw or "no_subject"), limit=max_subject_chars
            )
            conv_label = _format_datetime_label(conv_dt)
            conv_prefix = _order_prefix(conv_index, total_conversations)
            conv_dir_name = f"{conv_prefix}{conversation_shortcode}__{conv_label}__{conv_subject}"
            ...
            for msg_index, message_meta in enumerate(messages, start=1):
                ...
                msg_subject = _truncate_subject(_safe_filename(msg_subject_raw), limit=max_subject_chars)
                msg_label = _format_datetime_label(msg_dt)
                msg_prefix = _order_prefix(msg_index, total_messages)
                msg_dir_name = f"{msg_prefix}{message_shortcode}__{msg_label}__{msg_subject}"
                ...
                if not _copy_message_cache(cache_dir, msg_out_dir):
                    print(colored(f"Missing cache for message {message_id} ({message_shortcode}).", "red"))
                    return -1
            ...
            _write_json(os.path.join(conv_out_dir, "conversation_index.json"), conv_index_payload)
