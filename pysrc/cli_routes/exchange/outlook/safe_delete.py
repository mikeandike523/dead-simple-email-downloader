...
def _wrap_with_prefix(prefix, text, width):
    ...
    return textwrap.fill(
        text, width=width, initial_indent=prefix, subsequent_indent=subsequent
    )


def impl_exchange_outlook_safe_delete(
    exact_sender,
    exact_subject,
    ...
    if not exact_sender or not exact_subject:
        print(colored("exact-sender and exact-subject are required (or use --prompt).", "red"))
        return -1
    ...
    if not assume_yes:
        confirm = click.confirm(
            f"{action_label} {total} message(s) with safe-delete now?", default=False
        )
        if not confirm:
