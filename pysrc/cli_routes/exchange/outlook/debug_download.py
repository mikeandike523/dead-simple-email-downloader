...
from pysrc.helpers.exchange.paths import EXCHANGE_OUTLOOK_CACHE as CACHE_ROOT
...
def _features_for_message_dir(message_dir: str) -> Set[str]:
    ...
    has_file_attachments = _dir_has_entries(os.path.join(attachments_root, "files"), want_files=True)
    has_item_attachments = _dir_has_entries(os.path.join(attachments_root, "items"))
    has_link_attachments = _dir_has_entries(os.path.join(attachments_root, "links"), want_files=True)
...
def impl_exchange_outlook_debug_download(raw_features: Iterable[str], build_index: bool) -> int:
