...
export function signState(payload: { n: string; ts: number; pt: string; pr?: string; pd?: string }) {
...
export function verifyState(signed: string, maxAgeSec = 600) {
  ...
  const parsed = JSON.parse(body.toString()) as { n: string; ts: number; pt: string; pr?: string; pd?: string };
